...
from ...models.connection_body import ConnectionBody
...
) -> ConnectionTestResponse | HTTPExceptionResponse | HTTPValidationError | None:
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ConnectionTestResponse | HTTPExceptionResponse | HTTPValidationError]:
    return Response(
...
def sync_detailed(
    *,
    ...
    as some hook classes tries to find out the `conn` from their __init__ method & errors out if not
    found.
    It also deletes the conn id env connection after the test.

    Args:
        body (ConnectionBody): Connection Serializer for requests body.
...
def sync(
    *,
    ...
    Returns:
        ConnectionTestResponse | HTTPExceptionResponse | HTTPValidationError
    """
