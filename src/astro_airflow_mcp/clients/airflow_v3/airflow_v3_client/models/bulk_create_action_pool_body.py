...
if TYPE_CHECKING:
    from ..models.pool_body import PoolBody


T = TypeVar("T", bound="BulkCreateActionPoolBody")


@_attrs_define
class BulkCreateActionPoolBody:
    """
    Attributes:
        action (Literal['create']): The action to be performed on the entities.
        entities (list[PoolBody]): A list of entities to be created.
        action_on_existence (BulkActionOnExistence | Unset): Bulk Action to be taken if the entity already exists or
            not.
    """

    action: Literal["create"]
    entities: list[PoolBody]
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        for entities_item_data in _entities:
            entities_item = PoolBody.from_dict(entities_item_data)
        ...
        bulk_create_action_pool_body = cls(
            action=action,
            entities=entities,
            action_on_existence=action_on_existence,
        )

        return bulk_create_action_pool_body
