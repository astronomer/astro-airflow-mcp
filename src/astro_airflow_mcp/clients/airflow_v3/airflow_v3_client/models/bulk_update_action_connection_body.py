...
T = TypeVar("T", bound="BulkUpdateActionConnectionBody")


@_attrs_define
class BulkUpdateActionConnectionBody:
    """
    Attributes:
        action (Literal['update']): The action to be performed on the entities.
        entities (list[ConnectionBody]): A list of entities to be updated.
        action_on_non_existence (BulkActionNotOnExistence | Unset): Bulk Action to be taken if the entity does not
            exist.
    """

    action: Literal["update"]
    entities: list[ConnectionBody]
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        for entities_item_data in _entities:
            entities_item = ConnectionBody.from_dict(entities_item_data)
        ...
        bulk_update_action_connection_body = cls(
            action=action,
            entities=entities,
            action_on_non_existence=action_on_non_existence,
        )

        return bulk_update_action_connection_body
